...
proc usage() =
  echo "Usage: threadtools_detector_worker_pooled_probe <hef> <raw-input> [loops] [slots] [queue] [score-threshold]"
  echo ""
  echo "This variant pre-fills a threadtools Pool[seq[byte]] and submits PoolItem buffers."
  echo "The raw input is copied only while pre-filling the pool, not on every submit."
  echo ""
  echo "Example:"
  echo "  threadtools_detector_worker_pooled_probe yolov11n.hef dog_640x640x3.raw 100 2 4 0.25"

proc addInputBuffer(pool: Pool[seq[byte]]; templateBuf: openArray[byte]) =
  var buf = newSeq[byte](templateBuf.len)
  if templateBuf.len > 0:
    copyMem(addr buf[0], unsafeAddr templateBuf[0], templateBuf.len)

  let addRes = pool.addMove(buf)
  if addRes.isErr:
    quit(&"pool.addMove failed: {addRes.error}", QuitFailure)

proc submitFromPool(
  worker: ThreadtoolsDetectorWorker;
  pool: Pool[seq[byte]];
  requestId: int;
  threshold: float32;
  userDataBase: uint64
...
proc main() =
  ...
  let poolRes = newPool[seq[byte]](queueSize)
  if poolRes.isErr:
    quit(&"newPool failed: {poolRes.error}", QuitFailure)
  let inputPool = poolRes.get()

  for _ in 0 ..< queueSize:
    inputPool.addInputBuffer(inputTemplate)
  ...
  const userDataBase = 20_000'u64

  let started = getMonoTime()

  for _ in 0 ..< initial:
    submitFromPool(worker, inputPool, submitted, threshold, userDataBase)
    inc submitted
  ...
  echo ""
  echo "Threadtools detector worker pooled summary:"
